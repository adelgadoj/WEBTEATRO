<?php
    ...
    if(!empty($_POST["btn_envio"])){
        if(!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["nombre"]) && !empty($_POST["p_ape"]) && !empty($_POST["s_ape"]) && !empty($_POST["email"]) && !empty($_POST["genero"]) && !empty($_POST["dni"]) && !empty($_POST["telefono"]) && !empty($_POST["f_nac"])){
            ...
            $f_nac = $_POST["f_nac"];
            $result = $conexion->query("SELECT MAX(id_persona) FROM teatro.persona");
            $row = $result->fetch_assoc();
            $id_nuevo = $row['MAX(id_persona)'];
            $id_nuevo++;
            $conexion->query("INSERT INTO teatro.persona (id_persona,nombres,p_apel,s_apel,email,genero,DNI) VALUES ('$id_nuevo','$nombre','$p_ape','$s_ape','$email','$genero','$dni')");
            $conexion->query("INSERT INTO teatro.cliente (id_persona,f_nac,usuario,clave)
            VALUES ('$id_nuevo','$f_nac','$username','$password')");
            $conexion->query("INSERT INTO teatro.persona_telefono (id_persona,telefono) VALUES ('$id_nuevo','$telefono')");
