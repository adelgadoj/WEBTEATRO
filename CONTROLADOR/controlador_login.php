<?php
    ...
    if(!empty($_POST["btn_envio_login"])){
        if(!empty($_POST["username_login"]) && !empty($_POST["password_login"])){
            ...
            $sql = $conexion->query("SELECT persona.id_persona,CONCAT(persona.nombres,' ',persona.p_apel,' ',persona.s_apel) AS Nombres,persona.email,persona.DNI, cliente.f_nac FROM teatro.persona, teatro.cliente WHERE cliente.usuario = '$usuario' AND cliente.clave = '$password'");
